...
| **[pi-baml-runtime](packages/baml-runtime)** | Runtime boundary, BAML runtime adapter, and shared `ModelHandle` types |
...
`pi-agent-core` cleanly supports injected runtimes through the `pi-baml-runtime` boundary. `pi-coding-agent` is mid-migration: it already uses handle-based model identity and some runtime-boundary abstractions, but it still retains `pi-ai` compatibility and runtime-fallback seams in current execution paths. The current architecture is migration-in-progress, not a full `pi-ai` removal.
